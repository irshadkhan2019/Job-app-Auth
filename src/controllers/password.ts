...
import { BadRequestError, IAuthDocument, IEmailMessageDetails } from '@irshadkhan2019/job-app-shared'
...
//   if user with email exists then send him pass reset email 
// create token for verfication
...
//   send to rabbitmq queueu
  await publishDirectMessage(
    authChannel,
    'jobber-email-notification', //xhnge
    'auth-email', //rtngkey
...
export async function resetPassword(req: Request, res: Response): Promise<void> {
  ...
  const { token } = req.params; //reset_password?token=${randomCharacters}
...
export async function changePassword(req: Request, res: Response): Promise<void> {
  ...
  if (currentPassword!==newPassword) {
    throw new BadRequestError('Password must match', 'Password changePassword() method error');
  }
